...
$sql2 = "SELECT COUNT(*) as NbSonore FROM test WHERE numeroUtilisateur = '$id' AND  capteur = 'Sonore'";
$res2 = $db->prepare($sql2);
$res2->execute();
$resultat2 = $res2->fetch();
$nbTotalTest = $resultat2['NbSonore'];

switch ($_SESSION['langue']) {
    case 'francais':
        ?>
        <div id="titre"> Mes tests de mesure de réaction à un stimulus sonore</div>
        <?php
        echo "<div id='sous_titre'>" . ' Nombre de test(s) effectué(s) : ' . $nbTotalTest . "</div>";
        break;
    case 'anglais':
        ?>
        <div id="titre">My tests of reaction to a sound stimulus</div>
        <?php
...
if ($nbTotalTest == 0) {
    if ($_SESSION['langue'] == 'francais') {
        echo '<div class="Texte"> Vous n\'avez effectué aucun test de réaction à un stimulus sonore! N\'attendez plus ! </div>';
    } else {
        echo '<div class="Texte"> You have not carried out any test of reaction to a sound stimulus! Do not wait any longer ! </div>';
    }
} else {
    $sql3 = "SELECT COUNT(*) as NbTestValide FROM test WHERE numeroUtilisateur = '$id' AND capteur = 'Sonore' AND validation = 'oui'";
    ...
    switch ($nbTotalTest) {
        ...
        case $nbTotalTest % 2 == 0:
            ...
            capteur = 'Sonore' ORDER BY dateTest DESC LIMIT 0,$moitie ");
            $allIDGauchePair->execute();

            $allIDDroitePair = $db->prepare("SELECT * FROM test WHERE numeroUtilisateur = '$id' 
            AND capteur = 'Sonore' ORDER BY dateTest DESC LIMIT $moitie,$nbTotalTest ");
